...
//Signed: Lim Keonho Section: 01 Student Number: 21800612
//  listnode.cpp
...
pNode push_front(pNode p, int val) {
	...
	//cout << "your code here \n";
    pNode n;
    if(empty(p)) return n = new Ndoe{val, nullptr};
...
pNode push_back(pNode p, int val) {
	...
	//cout << "your code here \n";
    pNode node;
    if (empty(list)) return node = new Node{val, nullptr};
    pNode x = last(p);
...
pNode pop(pNode p, int val) {
	...
	//cout << "your code here \n";
    pNode x = p;
    while(x->next->item==val) x = x->next;
    pNode y = x->next;
    x -> next = x->next->next;
    delete y;
...
void show(pNode p, bool all, int pmax) {
	...
	//cout << "your code here \n";
    curr = p;
    for(i=0; i<pmax; i++, curr = curr->next){
        cout << "->" << curr->item;
    }
    cout << endl;
    cout << "\n...left out...\n";
    ...
    for(i=0; i<N-(pmax*2); i++) curr = curr->next;
    //cout << "your code here \n";
    for(i=0; i<pmax; i++, curr = curr->next) cout << "->" << curr->item;
    cout << "\n";
    return;
}
