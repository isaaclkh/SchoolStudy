...
int main(int argc, char *argv[]) {
    ...
    if(N <= STARTING_SAMPLES){
        cout << "Enter a number much larger than 500" << endl;
        return EXIT_FAILURE;
    }
    ...
    //cout << "your code here\n";
    profiling(quickSort, list, N);
    
    cout << "\n\tquickSort(): already sorted - worst case." << endl;
    int k = N - 1;
    for (int i = 0; i < N; i++) list[i] = k--;    // reversed sequence
    profiling(quickSort, list, N, false);

    // For the worst-case quicksort() testing
    //cout << "\n\tquickSort(): already sorted - worst case." << endl;
    //cout << "your code here\n";

    delete[] list;// deallocate resources
