...
void grow(heap p, int key) {
	...
	//cout << "your code here\n";
	(p->N) += 1;
	if(full(p)) reserve(p, (p->capacity)*2);
	p->nodes[p->N] = key;
	swim(p, p->N);
...
void trim(heap p) {
	...
	if(p->N==p->capacity/4) reserve(p, (p->capacity)/2);
	if(p->capacity == 1) p->capacity = 2;
...
int contains(heap p, int key) {
	...
	//cout << "your code here\n";
	for(int i=1; i<p->N; i++) if(p->nodes[i] == key) return 1;
...
void heapsort(heap p) {
    ...
    if(p->comp==more){
        for(int i=1, j=2; j<=p->N; i++, j++){
            if(more(p, i, j)) ::swap(p, i, j);
        }
    }
    
    else {
        for(int i=1, j=2; j<=p->N; i++, j++){
            if(::less(p, i, j)) ::swap(p, i, j);
        }
    }
...
void growN(heap p, int count, bool heapOrdered) {
	...
	//cout << "your code here\n";
	int keys[count];
	int maxi = maximum(p) + count;
	int arr[count];
	srand((unsigned)time(NULL));

	for(int i=0; i<count; i++){
		int val = rand()%(maxi-max+1)+max;
		arr[i] = val;
	}

	for(int j=0; j<count; j++) insertFunc(p, arr[j]);
...
void trimN(heap p, int count, bool heapOrdered) {
	...
	//cout << "your code here\n";
	if(count>size(p)) count = size(p);
	void(*func)(heap h) = heapOrdered ? trim : trimCBT;

	for(int i=0; i < count; i++) func(p);
