...
 Signed: _Lim Keonho_ Student Number: _21800612_
...
tree trim(tree node, int key) {
	...
	if (key < node->key) // if node to trim is in left subtree.
		...
		if (node->left && node->right) {
			...
			int val = x -> key;
			node->key = val;
			node->right = trim(node->right, x->key); 
...
tree trimplus(tree node, int key) {
	...
	const int hl = height(node->left);
	const int hr = height(node->right);
	...
	if (key < node->key)
		...
		if (node->left && node->right) {
			...
			if(hl>hr){
				...
				int val = x->key;
				node->key = val;
				node->left = trim(node->left, x->key);
			}
			else{
				tree x = succ(node);
				int val = x -> key;
...
bool isBST(tree root) {
	...
	tree min = minimum(root);
	tree max = maximum(root);
	int result = _isBST(root, min->key, max->key);    // an alternative
	return result;
}
...
tree rebalanceTree(tree node) {
	...
	// your code here // O(n)
 	...
 	return buildAVL(vect.data(), vect.size()); // O(n)
