...
class ApplicationState extends ChangeNotifier {
  ...
  Future<void> addMessageToGuestBook(String message) {
    if (!_loggedIn) {
  ...
  Future<void> deleteMessageToGuestBook(String message) {
    if (!_loggedIn) {
      throw Exception('Must be logged in');
    }
                ...
                StyledButton(
                  onPressed: () async {
                    if (_formKey.currentState!.validate()) {
                      var fromDate = DateFormat('yyyy-MM-dd HH:mm:ss.sss').format(DateTime.now());
        ...
        for (var message in widget.messages)
          Row(
            ...
            children: [
              ...
              if(message.name == currentUser?.displayName)
                Container(
                  child: IconButton(
                    icon : const Icon(Icons.delete_outline),
                    onPressed: ()  async {
                      await widget.deleteMessage(message.message);
                    },
