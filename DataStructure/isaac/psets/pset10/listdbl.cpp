...
assistance in the completion of this assignment. Signed: Keonho Lim
...
pNode half(pList p) {  // method 1
	...
	if(N%2==0)
		for(int i=0; i<N/2+1; i++) node = node->next;
	else
...
pNode half(pNode lo, pNode hi) {  // method 3 - rabbit and turtle
	//cout << "your code here. \n";
 	if (lo == hi || lo->next == hi) return lo;
	
	pNode rabbit = lo;
	pNode turtle = lo;

	while (rabbit != hi) { 
		if(rabbit->next==hi) break;
		rabbit = rabbit->next->next; 
		turtle = turtle->next;
	}
	return turtle;
...
void pop_all(pList p, int val) {
	...
	for (pNode c = begin(p); c != end(p);) {
        pNode x = c->next;
		if (c->item == val) erase(p, c);
		c = x;
	} // faster version but with bugs
#else
...
void shuffle(pList p) {
	...
	while(que!=half(p)->prev) que = que->next;
	que->next = nullptr;
	que = begin(p);
...
bool sorted(pList p, int(*comp)(int a, int b)) {
	...
	for (pNode c = x->next; c != end(p); c = c->next) {
		// your code here; return false as soon as out of order found 
		if(comp(value, c->item)>0) return false;
...
void push_sorted(pList p, int val) {
	...
	if(sorted(p)!=true) cout << "Sort first"<<endl;
	
	if(ascending(x->item, x->next->item)<0){
		pNode a = _more(p, val);
		insert(a, val);
	}
...
void push_sortedN(pList p, int N) {
	...
	bool answer = sorted(p, ascending);
...
void push_sortedNlog(pList p, int N) {
	...
	pNode listval = begin(p);

	srand((unsigned)time(NULL));

	for(int i=0; i<N; i++) vals[i] = rand() % range;
	...
	for(int i=0; i<N; i++){
		...
		if(answer){
			if(listval->item>vals[i]) insert(listval, vals[i]);
			else listval = listval->next;
			
			if(listval==end(p)) insert(listval->next, vals[i]);
		}
